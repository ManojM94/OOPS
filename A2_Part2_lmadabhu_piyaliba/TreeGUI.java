...
//      Need to code actionPerformed() for deleteButton and 
//      undoButton.   OK to add extra fields in TreeGUI 
//      to support you coding.
//
		...
		deleteButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
			 ...
			 // missing code to be filled in by you
				...
				tr = tree.clone();
				try {
				b = tree.delete(Integer.parseInt(s));
				if(b) {
					tm.set_state(tr);
					outputPanel.drawTree(tree);
				}
				else {
					if(tree.left==null&&tree.right==null&&tree.value==Integer.parseInt(s))
					{	tree=null;
					tm.set_state(tr);
					outputPanel.clearPanel();}
					else {
